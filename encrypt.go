...
//     32      1         8      ~ len(data)
func EncryptData(password []byte, data []byte) ([]byte, error) {
...
func DecryptData(password []byte, data io.Reader) ([]byte, error) {
	var (
		salt  [32]byte
		id    [1]byte
	...
	case c20p1305:
		stream, err = sio.ChaCha20Poly1305.Stream(key)
	default:
		err = errors.New("svalkey: invalid AEAD algorithm ID")
	}
	if err != nil {
		return nil, err
	}
